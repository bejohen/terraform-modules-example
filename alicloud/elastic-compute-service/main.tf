locals {
  routes = {
    for x in var.routes :
    "${x.routes_type}/${x.routes_protocol}/${x.routes_nictype}/${x.routes_policy}/${x.routes_portrange}/${x.routes_cidrip}" => x
  }
}
...
resource "alicloud_security_group_rule" "allow_ssh" {
  ...
  cidr_ip                  = "10.192.0.0/16"
}

resource "alicloud_security_group_rule" "main" {
  for_each          = local.routes
  description       = "${each.value.routes_policy} ${each.value.routes_type} ${each.value.routes_cidrip}"
  type              = each.value.routes_type
  ip_protocol       = each.value.routes_protocol
  nic_type          = each.value.routes_nictype
  policy            = each.value.routes_policy
  port_range        = each.value.routes_portrange
  priority          = 1
  security_group_id = alicloud_security_group.main.id
  cidr_ip           = each.value.routes_cidrip
  depends_on = [ alicloud_security_group.main ]
}
